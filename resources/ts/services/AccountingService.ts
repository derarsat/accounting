...
export class AccountingService {
    ...
    async getPayments(page, perPage, sortBy, orderBy, start, end, type): Promise<Response> {
        let url = getUrl("payment") + `?page=${page}&per_page=${perPage}&sort=${sortBy}&order=${orderBy}&start=${start}&end=${end}&type=${type}`
        return await this.http.get(url)
    }
}
